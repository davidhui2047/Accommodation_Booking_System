...
<main class="flex-shrink-0">
  <p></p>
  <div class="container">
    <form id="manager_edit_user" action="./backend/manager_update_user.php?uid=<?php echo $id; ?>" method="POST" enctype="multipart/form-data">
        <p>
      ...
      <p>
        <lable>Access level</lable> <!-- Change access level from the one that is stored in the SQL database -->
      ...
      <div class="saveCancel">
        <button class="btn btn-outline-danger" onclick="location.href='manager_user.php'" type="button">Cancel</button> <!-- Cancel button. Takes the user manage users page-->
        <button class="btn btn-outline-info" onclick="location.href='manager_reset_pw.php?id=<?php echo $id; ?>'" type="button">Reset password</button> <!-- button that will allow the user to reset the current password -->
        <input type="submit" name="submit" id="submit" form="manager_edit_user" value="Save" class="btn btn-outline-primary"></input> <!-- Submit button that will perform Action -->
      </div>
    </form>
  </div>
  </div>
